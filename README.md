...
Use `--priority` to set an ensemble's relative share of worker time. The
default is `100`; background work can use `--priority 5` to yield to normal
ensembles while still using idle workers. Agents apply this weight between
tests, so it does not preempt running tests or reserve a fixed worker count.
