...
class UploadJobForm(FlaskForm):
    ...
    fail_fast = IntegerField(
        "Fail Fast", default=10, validators=[NumberRange(min=0, max=1000)]
    )
    allow_multiple = BooleanField("Allow Multiple Ensembles for User", default=False)
    no_max_runs = BooleanField("Test Continuously", default=False)
    no_fail_fast = BooleanField("No Limit on Failures", default=False)
    max_runs = IntegerField(
        "Max Runs", default=20 * 1000, validators=[NumberRange(min=0, max=1000 * 1000)]
    )
    priority = IntegerField(
        "Priority", default=100, validators=[NumberRange(min=1, max=1000)]
    )
    timeout = IntegerField(
        "Timeout", default=5400, validators=[NumberRange(min=100, max=43200)]
    )
...
def job():
    ...
    return render_template("job.html", user=current_user, ensembles=ensemble_list)
...
def joblist():
    ...
    app.logger.info(
        "joblist: stopped: {}  sanity: {}  usersort: {}".format(
    ...
    return render_template("joblist.txt", user=current_user, ensembles=ensemble_list)
...
def jobstop():
    ...
    sanity = (sanity_arg == "true") or (sanity_arg == "1")
    app.logger.info(
        "jobstop: id: {}  username: {}  sanity: {}".format(jobid, username, sanity)
    )
    joshua.stop_ensemble(jobid, username, sanity)
    return render_template(
        "joblist.txt",
        user=current_user,
        ensembles=joshua.get_active_ensembles(False, sanity, username),
    )
...
def jobtail():
    ...
    jobfile = os.path.join(
        app.config["JOSHUA_UPLOAD_FOLDER"], "tail__" + fileid + ".log"
    )
    app.logger.info(
        "jobtail: id: {}  errors: {}  xml: {}  simple: {}".format(
            jobid, errorsonly, xml, simple
        )
    )
    stdout_backup = sys.stdout
    sys.stdout = open(jobfile, "w")
    joshua.tail_ensemble(
        jobid,
        raw=raw,
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        filepath = os.path.join(
            app.config["JOSHUA_UPLOAD_FOLDER"], secure_filename(current_user.username)
        )
        ...
        flash(
            "Uploaded:  user: {}   package: {}".format(current_user.username, filename)
        )
        app.logger.info(
            "Uploaded:  user: {}   package: {}".format(current_user.username, filename)
        )
        ...
        ensemble_id = joshua_model.create_ensemble(
            properties["username"], properties, tarfile, False
        )
        app.logger.info(
            "Ensemble {} created with properties: {}!".format(ensemble_id, properties)
        )
