...
class UploadJobForm(Schema):
    fail_fast = fields.Int(default=10, missing=10, validate=Range(min=0, max=1000))
    no_fail_fast = fields.Bool(default=False, missing=False)
    max_runs = fields.Int(
        default=1000, missing=1000, validate=Range(min=0, max=1000000)
    )
    priority = fields.Int(default=100, missing=100, validate=Range(min=1, max=1000))
    timeout = fields.Int(default=5400, missing=5400, validate=Range(min=100, max=43200))
...
def list_ensembles():
    ...
    app.logger.info(
        "joblist: stopped: {} sanity: {} usersort: {}".format(stopped, sanity, usersort)
    )
...
def upload_ensemble():
    ...
    if not request_files:
        ...
        return {
            "message": "api_upload: Missing uploaded file  request: {}".format(
                request_data
            )
        }, 400
    ...
    try:
        ...
        return {"message": f"api_upload: Post field validation error: {err}"}, 422
    fileobj = request_files["file"]
    filename = secure_filename(fileobj.filename)
    filepath = os.path.join(
        app.config["JOSHUA_UPLOAD_FOLDER"], secure_filename(properties["username"])
    )
    ...
    if not tarfile.is_tarfile(saved_file):
        ...
        app.logger.info(f"api_upload: not a valid tar file: {saved_file}")
    ...
    with open(saved_file, "rb") as file:
        ...
        ensemble_id = joshua_model.create_ensemble(
            properties["username"], properties, file, False
        )
        app.logger.info(
...
def stop_ensemble(ensemble):
    ...
    sanity = properties["sanity"] if properties and "sanity" in properties else True
